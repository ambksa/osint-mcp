...
export class DataLoaderManager implements AppModule {
  ...
  private polymarketIntelSnapshot: PolymarketIntelSnapshot | null = null;
  ...
  async loadAllData(): Promise<void> {
    ...
    if (SITE_VARIANT !== 'happy') {
      ...
      if (SITE_VARIANT === 'full' || SITE_VARIANT === 'finance') {
        ...
        tasks.push({ name: 'polymarketIntel', task: runGuarded('polymarketIntel', () => this.loadPolymarketIntel()) });
      }
    }
    ...
    if (SITE_VARIANT !== 'happy' && this.ctx.mapLayers.construction) tasks.push({ name: 'construction', task: runGuarded('construction', () => this.loadConstructionSites()) });
    ...
    try {
      switch (layer) {
        ...
        case 'flights':
          ...
          break;
        case 'construction':
          await this.loadConstructionSites();
          break;
        case 'geoFilters':
          break;
        case 'military':
  ...
  async loadPolymarketIntel(): Promise<void> {
    const panel = this.ctx.panels['polymarket'] as PredictionPanel | undefined;
    if (!panel) return;
    try {
      const snapshot = await fetchPolymarketIntel();
      this.polymarketIntelSnapshot = snapshot;
      panel.renderIntel(snapshot);
      this.ctx.statusPanel?.updateFeed('Polymarket Intel', { status: 'ok', itemCount: snapshot.markets.length });
      this.ctx.statusPanel?.updateApi('Polymarket Intel', { status: 'ok' });
      dataFreshness.recordUpdate('polymarket_intel', snapshot.markets.length);
    } catch (error) {
      this.ctx.statusPanel?.updateFeed('Polymarket Intel', { status: 'error', errorMessage: String(error) });
      this.ctx.statusPanel?.updateApi('Polymarket Intel', { status: 'error' });
      dataFreshness.recordError('polymarket_intel', String(error));
    }
  }

  async loadPolymarketLiveTrades(): Promise<void> {
    const panel = this.ctx.panels['polymarket'] as PredictionPanel | undefined;
    if (!panel) return;
    try {
      const live = await fetchPolymarketLiveTrades(250);
      const base = this.polymarketIntelSnapshot;
      if (base) {
        const merged: PolymarketIntelSnapshot = {
          ...base,
          trades: live.trades,
        ...
        this.polymarketIntelSnapshot = merged;
        panel.renderIntel(merged);
      } else {
        await this.loadPolymarketIntel();
      }
      this.ctx.statusPanel?.updateFeed('Polymarket Intel', { status: 'ok', itemCount: live.trades.length });
      this.ctx.statusPanel?.updateApi('Polymarket Intel', { status: 'ok' });
      dataFreshness.recordUpdate('polymarket_intel', live.trades.length);
    } catch (error) {
      this.ctx.statusPanel?.updateFeed('Polymarket Intel', { status: 'error', errorMessage: String(error) });
      this.ctx.statusPanel?.updateApi('Polymarket Intel', { status: 'error' });
      dataFreshness.recordError('polymarket_intel', String(error));
  ...
  async loadConstructionSites(): Promise<void> {
    try {
      const { sites, newSites } = await fetchConstructionSites();
      this.ctx.map?.setConstructionSites(sites);
      this.ctx.map?.setLayerReady('construction', sites.length > 0);
      dataFreshness.recordUpdate('construction', sites.length);

      if (newSites.length > 0) {
        const signalSites = newSites.length > 200 ? newSites.slice(0, 200) : newSites;
        signalAggregator.ingestConstructionSites(signalSites.map(s => ({
          lat: s.lat,
          lon: s.lon,
          name: s.name,
          region: s.region,
      ...
      updateAndCheck([
        { type: 'construction', region: 'global', count: newSites.length },
      ]).then(anomalies => {
        if (anomalies.length > 0) {
          signalAggregator.ingestTemporalAnomalies(anomalies);
        }
      ...
      console.warn('[App] Construction load failed:', e);
      this.ctx.map?.setLayerReady('construction', false);
      dataFreshness.recordError('construction', String(e));
    }
  }
