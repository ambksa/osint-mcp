// Single backend target for web runtime (no local bridge dependency).
const WS_API_URL = 'https://h1dr4-worldosint-dirlk4bbwa-ew.a.run.app';
...
const WEB_API_PATTERN = /^\/api\//;

function isAllowedRedirectTarget(url: string): boolean {
  try {
    const parsed = new URL(url);
    // Web deployments must be able to point to external API backends (Cloud Run/custom domains).
    // Allow HTTPS everywhere; allow HTTP only for local development.
...
export function installWebApiRedirect(): void {
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    if (typeof input === 'string' && WEB_API_PATTERN.test(input)) {
      return nativeFetch(`${API_BASE}${input}`, init);
    }
    if (input instanceof URL && input.origin === window.location.origin && WEB_API_PATTERN.test(input.pathname)) {
      return nativeFetch(new URL(`${API_BASE}${input.pathname}${input.search}`), init);
    }
    if (input instanceof Request) {
      const u = new URL(input.url);
      if (u.origin === window.location.origin && WEB_API_PATTERN.test(u.pathname)) {
