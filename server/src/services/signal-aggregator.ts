...
export type SignalType =
  ...
  | 'construction'          // New construction / major works
...
class SignalAggregator {
  ...
  /**
   * Ingest construction activity signals (OSM construction features).
   */
  ingestConstructionSites(sites: Array<{ lat: number; lon: number; name: string; region: string }>): void {
    this.clearSignalType('construction');

    for (const site of sites) {
      const code = this.coordsToCountry(site.lat, site.lon) || normalizeCountryCode(site.region);
      this.signals.push({
        type: 'construction',
        country: code,
        countryName: site.region,
        lat: site.lat,
        lon: site.lon,
        severity: 'low',
        title: `Construction activity: ${site.name}`,
        timestamp: new Date(),
      });
    }
    this.pruneOld();
