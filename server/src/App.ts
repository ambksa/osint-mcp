...
export class App {
  ...
  constructor(containerId: string) {
    ...
    if (storedVariant !== currentVariant) {
      ...
      if (currentVariant === 'happy') {
        const unhappyLayers: (keyof MapLayers)[] = ['conflicts', 'bases', 'hotspots', 'nuclear', 'irradiators', 'sanctions', 'military', 'protests', 'pipelines', 'waterways', 'ais', 'flights', 'spaceports', 'minerals', 'natural', 'fires', 'satellite', 'geoFilters', 'construction', 'outages', 'cyberThreats', 'weather', 'economic', 'cables', 'datacenters', 'ucdpEvents', 'displacement', 'climate'];
    ...
    if (initialUrlState.layers) {
      if (currentVariant === 'tech') {
        const geoLayers: (keyof MapLayers)[] = ['conflicts', 'bases', 'hotspots', 'nuclear', 'irradiators', 'sanctions', 'military', 'protests', 'pipelines', 'waterways', 'ais', 'flights', 'spaceports', 'minerals', 'construction', 'geoFilters'];
  ...
  private setupRefreshIntervals(): void {
    ...
    if (SITE_VARIANT === 'full' || SITE_VARIANT === 'finance') {
      ...
      this.refreshScheduler.scheduleRefresh('polymarketIntel', () => this.dataLoader.loadPolymarketIntel(), REFRESH_INTERVALS.polymarketIntel);
      this.refreshScheduler.scheduleRefresh('polymarketTrades', () => this.dataLoader.loadPolymarketLiveTrades(), REFRESH_INTERVALS.polymarketTrades);
    }
