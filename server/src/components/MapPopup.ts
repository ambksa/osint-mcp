...
export class MapPopup {
  ...
  private normalizeSeverityBadge(
    severity: unknown,
    fallbackClass: 'low' | 'elevated' | 'high' = 'low',
  ): { cssClass: 'low' | 'elevated' | 'high'; label: string } {
    const raw = String(severity ?? '').trim();
    const numeric = Number(raw);
    if (Number.isFinite(numeric)) {
      if (numeric >= 70) return { cssClass: 'high', label: `SEV ${Math.round(numeric)}` };
      if (numeric >= 40) return { cssClass: 'elevated', label: `SEV ${Math.round(numeric)}` };
      return { cssClass: 'low', label: `SEV ${Math.round(numeric)}` };
    }

    const normalized = raw.toLowerCase();
    if (['critical', 'severe', 'high', 'total'].includes(normalized)) {
      return { cssClass: 'high', label: raw.toUpperCase() || 'HIGH' };
    }
    if (['elevated', 'moderate', 'medium', 'major', 'partial'].includes(normalized)) {
      return { cssClass: 'elevated', label: raw.toUpperCase() || 'ELEVATED' };
    }
    if (['low', 'minor', 'normal', 'none'].includes(normalized)) {
      return { cssClass: 'low', label: raw.toUpperCase() || 'LOW' };
    }

    if (fallbackClass === 'high') return { cssClass: 'high', label: raw.toUpperCase() || 'HIGH' };
    if (fallbackClass === 'elevated') return { cssClass: 'elevated', label: raw.toUpperCase() || 'ELEVATED' };
    return { cssClass: 'low', label: raw.toUpperCase() || 'LOW' };
  }
  ...
  private renderAisPopup(event: AisDisruptionEvent): string {
    const sev = this.normalizeSeverityBadge(event.severity, 'elevated');
    const severityClass = sev.cssClass;
    const severityLabel = escapeHtml(sev.label);
  ...
  private renderFlightPopup(delay: AirportDelayAlert): string {
    const sev = this.normalizeSeverityBadge(delay.severity, 'elevated');
    const severityClass = sev.cssClass;
    const severityLabel = escapeHtml(sev.label);
  ...
  private renderCyberThreatPopup(threat: CyberThreat): string {
    const sev = this.normalizeSeverityBadge(threat.severity, 'elevated');
    const severityClass = sev.cssClass;
    const sourceLabels: Record<string, string> = {
      feodo: 'Feodo Tracker',
      urlhaus: 'URLhaus',
    ...
    return `
      <div class="popup-header apt ${severityClass}">
        ...
        <span class="popup-badge ${severityClass}">${escapeHtml(sev.label)}</span>
        <button class="popup-close">×</button>
      </div>
      <div class="popup-body">
