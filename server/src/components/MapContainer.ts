...
export class MapContainer {
  ...
  public setConstructionSites(sites: ConstructionSite[]): void {
    if (this.useDeckGL) {
      this.deckGLMap?.setConstructionSites(sites);
    } else {
      this.svgMap?.setConstructionSites(sites);
    }
  }

  public setGeoFilterResults(features: GeoFilterFeature[], matches: GeoFilterMatch[]): void {
    if (this.useDeckGL) {
      this.deckGLMap?.setGeoFilterResults(features, matches);
    } else {
      this.svgMap?.setGeoFilterResults(features, matches);
    }
  }
  ...
  public setLayerEnabled(layer: keyof MapLayers, enabled: boolean): void {
    if (this.useDeckGL) {
      this.deckGLMap?.setLayerEnabled(layer, enabled);
    } else {
      this.svgMap?.setLayerEnabled(layer, enabled);
    }
  }

  public getBounds(): { west: number; south: number; east: number; north: number } | null {
    if (this.useDeckGL) {
      return this.deckGLMap?.getBounds() ?? null;
    }
    return this.svgMap?.getBounds() ?? null;
  }
