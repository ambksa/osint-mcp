...
const GEOFILTER_COLORS: Record<GeoFilterFeature['type'], [number, number, number, number]> = {
  roundabout: [59, 130, 246, 210],
  traffic_signals: [234, 88, 12, 210],
  stop_sign: [220, 38, 38, 210],
  street_sign: [16, 185, 129, 210],
  milestone: [217, 119, 6, 210],
  level_crossing: [139, 92, 246, 210],
  bridge: [14, 116, 144, 210],
};

const GEOFILTER_MATCH_COLOR: [number, number, number, number] = [250, 204, 21, 235];
...
export class DeckGLMap {
  ...
  private attributionEl: HTMLDivElement | null = null;
  ...
  private geoFilterFeatures: GeoFilterFeature[] = [];
  private geoFilterMatches: GeoFilterMatch[] = [];
  ...
  private satelliteSelecting = false;
  private satelliteSelectStart: { x: number; y: number } | null = null;
  private satelliteSelectBox: HTMLDivElement | null = null;
  private satelliteOverlay: HTMLDivElement | null = null;
  private satelliteOverlayControls: HTMLDivElement | null = null;
  private satelliteOverlayState: { live: boolean; date: string; source: 'auto' | 'arcgis' | 'naip' | 'sentinel' | 'gibs' | 'goes16_vis' } = {
    live: true,
    date: '',
    source: 'auto',
  };
  private satelliteOverlayBounds: { sw: maplibregl.LngLat; ne: maplibregl.LngLat } | null = null;
  private satelliteOverlayImg: HTMLImageElement | null = null;
  private satelliteOverlaySourceLabel: HTMLSpanElement | null = null;
  private satelliteOverlayDateLabel: HTMLSpanElement | null = null;
  private satelliteOverlayCandidates: Array<{ url: string; label: string }> = [];
  private satelliteOverlayCandidateIndex = 0;
  ...
  private static readonly SATELLITE_SOURCE_ID = 'satellite-gibs';
  private static readonly SATELLITE_LAYER_ID = 'satellite-gibs-layer';
  ...
  private setupSatelliteSelection(): void {
    ...
    canvas.addEventListener('mousedown', (e: MouseEvent) => {
      ...
      if ((e.target as HTMLElement)?.closest('.map-controls, .layer-toggles, .deckgl-time-slider, .map-popup')) return;

      this.satelliteSelecting = true;
      this.satelliteSelectStart = { x: e.offsetX, y: e.offsetY };
      this.maplibreMap?.dragPan.disable();

      const box = document.createElement('div');
      box.className = 'satellite-select-box';
      box.style.left = `${e.offsetX}px`;
      box.style.top = `${e.offsetY}px`;
      box.style.width = '1px';
      box.style.height = '1px';
      this.wrapperEl.appendChild(box);
      this.satelliteSelectBox = box;
    });

    canvas.addEventListener('mousemove', (e: MouseEvent) => {
      if (!this.satelliteSelecting || !this.satelliteSelectStart || !this.satelliteSelectBox) return;
      const x = e.offsetX;
      const y = e.offsetY;
      const left = Math.min(this.satelliteSelectStart.x, x);
      const top = Math.min(this.satelliteSelectStart.y, y);
      const width = Math.abs(this.satelliteSelectStart.x - x);
      const height = Math.abs(this.satelliteSelectStart.y - y);
      this.satelliteSelectBox.style.left = `${left}px`;
      this.satelliteSelectBox.style.top = `${top}px`;
      this.satelliteSelectBox.style.width = `${Math.max(width, 1)}px`;
      this.satelliteSelectBox.style.height = `${Math.max(height, 1)}px`;
    });

    const finalize = (e: MouseEvent) => {
      if (!this.satelliteSelecting || !this.satelliteSelectStart) return;
      this.satelliteSelecting = false;
      this.maplibreMap?.dragPan.enable();

      const start = this.satelliteSelectStart;
      const end = { x: e.offsetX, y: e.offsetY };
      this.satelliteSelectStart = null;

      if (this.satelliteSelectBox) {
        this.satelliteSelectBox.remove();
        this.satelliteSelectBox = null;
      }
      ...
      const p1 = this.maplibreMap.unproject([start.x, start.y]);
      const p2 = this.maplibreMap.unproject([end.x, end.y]);
      const sw = new maplibregl.LngLat(Math.min(p1.lng, p2.lng), Math.min(p1.lat, p2.lat));
      const ne = new maplibregl.LngLat(Math.max(p1.lng, p2.lng), Math.max(p1.lat, p2.lat));

      if (dx < minDrag && dy < minDrag) {
        // Treat as click focus: small bounds around the point
        const center = this.maplibreMap.unproject([end.x, end.y]);
        const delta = 0.05;
        const swc = new maplibregl.LngLat(center.lng - delta, center.lat - delta);
        const nec = new maplibregl.LngLat(center.lng + delta, center.lat + delta);
        this.openSatelliteOverlay({ sw: swc, ne: nec });
      } else {
        this.openSatelliteOverlay({ sw, ne });
      }
    };

    canvas.addEventListener('mouseup', finalize);
    canvas.addEventListener('mouseleave', (e: MouseEvent) => {
      if (this.satelliteSelecting) finalize(e);
    });
  }

  private getSatelliteDate(): string {
    // Use yesterday's UTC date to reduce chance of missing tiles.
  ...
  private getSatelliteLayerId(): string {
    return 'VIIRS_SNPP_CorrectedReflectance_TrueColor';
  }

  private buildGibsTileUrl(layerId: string, date: string): string {
    return `https://gibs.earthdata.nasa.gov/wmts/epsg3857/best/${layerId}/default/${date}/GoogleMapsCompatible_Level9/{z}/{y}/{x}.jpg`;
  }

  private buildGibsWmsUrl(layerId: string, date: string, bbox: [number, number, number, number], width: number, height: number, availability: 'best' | 'nrt' = 'best'): string {
    ...
    return `https://gibs.earthdata.nasa.gov/wms/epsg4326/${availability}/wms.cgi?${params.toString()}`;
  }

  private buildWmsUrl(baseUrl: string, layerId: string, bbox: [number, number, number, number], width: number, height: number): string {
    const [west, south, east, north] = bbox;
    const params = new URLSearchParams({
      service: 'WMS',
      request: 'GetMap',
  ...
  private buildNaipImageUrl(bbox: [number, number, number, number], width: number, height: number): string {
    const [west, south, east, north] = bbox;
    const params = new URLSearchParams({
      bbox: `${west},${south},${east},${north}`,
      bboxSR: '4326',
    ...
    return `https://services.nationalmap.gov/arcgis/rest/services/USGSNAIPImagery/ImageServer/exportImage?${params.toString()}`;
  }

  private buildArcGisWorldImageryUrl(bbox: [number, number, number, number], width: number, height: number): string {
    const [west, south, east, north] = bbox;
    const params = new URLSearchParams({
      bbox: `${west},${south},${east},${north}`,
      bboxSR: '4326',
  ...
  private buildGoesWmsUrl(layerId: string, bbox: [number, number, number, number], width: number, height: number): string {
    const base = 'https://wms1.nsstc.nasa.gov/geoserver/GOES/wms';
    return this.buildWmsUrl(base, layerId, bbox, width, height);
  }

  private isConus(bounds: { sw: maplibregl.LngLat; ne: maplibregl.LngLat }): boolean {
    const centerLat = (bounds.sw.lat + bounds.ne.lat) / 2;
    const centerLon = (bounds.sw.lng + bounds.ne.lng) / 2;
    return centerLat >= 24.396308 && centerLat <= 49.384358 && centerLon >= -124.848974 && centerLon <= -66.885444;
  }

  private loadSatelliteImage(candidates: Array<{ url: string; label: string }>): void {
    if (!this.satelliteOverlayImg) return;
    this.satelliteOverlayCandidates = candidates;
    this.satelliteOverlayCandidateIndex = 0;
    const statusEl = this.satelliteOverlayControls?.querySelector('.satellite-status') as HTMLSpanElement | null;
    if (statusEl) statusEl.textContent = 'Loading...';
    this.tryNextSatelliteCandidate();
  }

  private tryNextSatelliteCandidate(): void {
    if (!this.satelliteOverlayImg) return;
    if (this.satelliteOverlayCandidateIndex >= this.satelliteOverlayCandidates.length) {
      const statusEl = this.satelliteOverlayControls?.querySelector('.satellite-status') as HTMLSpanElement | null;
      if (statusEl) statusEl.textContent = 'Tile error';
      return;
    }
    const candidate = this.satelliteOverlayCandidates[this.satelliteOverlayCandidateIndex]!;
    if (this.satelliteOverlaySourceLabel) {
      this.satelliteOverlaySourceLabel.textContent = `Source: ${candidate.label}`;
    }
    this.satelliteOverlayImg.src = candidate.url;
  }

  private isUsCenter(bounds: { sw: maplibregl.LngLat; ne: maplibregl.LngLat }): boolean {
    const centerLat = (bounds.sw.lat + bounds.ne.lat) / 2;
    const centerLon = (bounds.sw.lng + bounds.ne.lng) / 2;
    // CONUS
    if (centerLat >= 24.396308 && centerLat <= 49.384358 && centerLon >= -124.848974 && centerLon <= -66.885444) return true;
    // Alaska
    if (centerLat >= 51.2 && centerLat <= 71.5 && centerLon >= -179.2 && centerLon <= -129.9) return true;
    // Hawaii
    if (centerLat >= 18.8 && centerLat <= 22.3 && centerLon >= -160.5 && centerLon <= -154.5) return true;
    return false;
  }

  private isSmallSelection(bounds: { sw: maplibregl.LngLat; ne: maplibregl.LngLat }): boolean {
    const latSpan = Math.abs(bounds.ne.lat - bounds.sw.lat);
    const lonSpan = Math.abs(bounds.ne.lng - bounds.sw.lng);
    return latSpan < 0.5 && lonSpan < 0.5;
  }

  private updateAttribution(): void {
    if (!this.attributionEl) return;
    const base = '© <a href="https://carto.com/attributions" target="_blank" rel="noopener">CARTO</a> © <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener">OpenStreetMap</a>';
    const satellite = this.state.layers.satellite || this.satelliteOverlayImg
      ? ' · <a href="https://earthdata.nasa.gov/gibs" target="_blank" rel="noopener">NASA GIBS</a>'
      : '';
    this.attributionEl.innerHTML = base + satellite;
  }

  private openSatelliteOverlay(bounds: { sw: maplibregl.LngLat; ne: maplibregl.LngLat }): void {
    this.satelliteOverlayBounds = bounds;
    if (!this.wrapperEl) return;
    ...
    if (!this.satelliteOverlay) {
      ...
      overlay.innerHTML = `
        <img class="satellite-overlay-img" alt="Satellite selection"/>
      `;
      this.wrapperEl.appendChild(overlay);
      this.satelliteOverlay = overlay;

      // Ensure overlay has size before initializing the map inside it.
      this.positionSatelliteOverlay();

      this.satelliteOverlayImg = overlay.querySelector('.satellite-overlay-img') as HTMLImageElement | null;
      if (this.satelliteOverlayImg) {
        this.satelliteOverlayImg.onload = () => {
          const statusEl = this.satelliteOverlayControls?.querySelector('.satellite-status') as HTMLSpanElement | null;
          if (statusEl) statusEl.textContent = 'Loaded';
        };
        this.satelliteOverlayImg.onerror = () => {
          this.satelliteOverlayCandidateIndex += 1;
          this.tryNextSatelliteCandidate();
        };
      }

      this.createSatelliteOverlayControls();
    } else {
      this.updateSatelliteOverlayLayer();
      this.syncSatelliteOverlay();
    }
  ...
  private positionSatelliteOverlay(): void {
    if (!this.maplibreMap || !this.satelliteOverlay || !this.satelliteOverlayBounds) return;

    const sw = this.satelliteOverlayBounds.sw;
    const ne = this.satelliteOverlayBounds.ne;
    const swPoint = this.maplibreMap.project([sw.lng, sw.lat]);
    const nePoint = this.maplibreMap.project([ne.lng, ne.lat]);
    ...
    this.satelliteOverlay.style.left = `${left}px`;
    this.satelliteOverlay.style.top = `${top}px`;
    this.satelliteOverlay.style.width = `${Math.max(width, 2)}px`;
    this.satelliteOverlay.style.height = `${Math.max(height, 2)}px`;

    if (this.satelliteOverlayControls) {
      const ctrlTop = top + height + 8;
      const ctrlLeft = left;
      const maxY = this.container.getBoundingClientRect().height - 40;
      this.satelliteOverlayControls.style.left = `${ctrlLeft}px`;
      this.satelliteOverlayControls.style.top = `${Math.min(ctrlTop, maxY)}px`;
    }
  }

  private createSatelliteOverlayControls(): void {
    if (!this.wrapperEl) return;
    const controls = document.createElement('div');
    controls.className = 'satellite-overlay-controls';
    controls.innerHTML = `
      <label class="satellite-toggle">
        <input type="checkbox" data-sat-live checked>
        <span>Live (Latest)</span>
      </label>
      <select class="satellite-source-select" data-sat-source>
        <option value="auto">Auto</option>
        <option value="arcgis">ArcGIS World Imagery</option>
        <option value="naip">NAIP (US)</option>
        <option value="sentinel">Sentinel-2 Cloudless</option>
        <option value="gibs">NASA GIBS</option>
        <option value="goes16_vis">GOES-16 Visible (CONUS)</option>
      </select>
      <input type="date" class="satellite-date" data-sat-date>
      <span class="satellite-status">Loading...</span>
      <span class="satellite-source">Source: —</span>
      <span class="satellite-date-label">Date: —</span>
      <button class="satellite-close">Close</button>
    `;
    this.wrapperEl.appendChild(controls);
    this.satelliteOverlayControls = controls;

    const liveToggle = controls.querySelector('[data-sat-live]') as HTMLInputElement | null;
    const dateInput = controls.querySelector('[data-sat-date]') as HTMLInputElement | null;
    const closeBtn = controls.querySelector('.satellite-close') as HTMLButtonElement | null;
    const statusEl = controls.querySelector('.satellite-status') as HTMLSpanElement | null;
    this.satelliteOverlaySourceLabel = controls.querySelector('.satellite-source') as HTMLSpanElement | null;
    this.satelliteOverlayDateLabel = controls.querySelector('.satellite-date-label') as HTMLSpanElement | null;
    const sourceSelect = controls.querySelector('[data-sat-source]') as HTMLSelectElement | null;

    if (dateInput) {
      const fallbackDate = this.getSatelliteDate();
      dateInput.value = fallbackDate;
      this.satelliteOverlayState.date = fallbackDate;
      dateInput.disabled = true;
    }

    liveToggle?.addEventListener('change', () => {
      this.satelliteOverlayState.live = Boolean(liveToggle.checked);
      if (dateInput) dateInput.disabled = this.satelliteOverlayState.live;
      if (statusEl) statusEl.textContent = 'Loading...';
      this.updateSatelliteOverlayLayer();
    });
    dateInput?.addEventListener('change', () => {
      this.satelliteOverlayState.date = dateInput.value;
      if (statusEl) statusEl.textContent = 'Loading...';
      this.updateSatelliteOverlayLayer();
    });
    sourceSelect?.addEventListener('change', () => {
      this.satelliteOverlayState.source = (sourceSelect.value as typeof this.satelliteOverlayState.source) || 'auto';
      if (statusEl) statusEl.textContent = 'Loading...';
      this.updateSatelliteOverlayLayer();
    });
    closeBtn?.addEventListener('click', () => this.closeSatelliteOverlay());
  }

  private updateSatelliteOverlayLayer(): void {
    if (!this.satelliteOverlayImg || !this.satelliteOverlayBounds) return;

    const layerName = this.getSatelliteLayerId();
    const date = this.satelliteOverlayState.live
      ? this.getSatelliteDate()
      : (this.satelliteOverlayState.date || this.getSatelliteDate());

    const pixelRatio = Math.min(window.devicePixelRatio || 1, 2);
    const width = Math.max(Math.round(this.satelliteOverlay.clientWidth * pixelRatio), 512);
    const height = Math.max(Math.round(this.satelliteOverlay.clientHeight * pixelRatio), 512);
    const maxDim = 2048;
    const clampedWidth = Math.min(width, maxDim);
    const clampedHeight = Math.min(height, maxDim);
    const bbox: [number, number, number, number] = [
      this.satelliteOverlayBounds.sw.lng,
      this.satelliteOverlayBounds.sw.lat,
      this.satelliteOverlayBounds.ne.lng,
      this.satelliteOverlayBounds.ne.lat,
    ];

    const isUs = this.isUsCenter(this.satelliteOverlayBounds);
    const isSmall = this.isSmallSelection(this.satelliteOverlayBounds);

    const candidates: Array<{ url: string; label: string }> = [];

    const source = this.satelliteOverlayState.source;
    const forceSource = source !== 'auto';

    if (source === 'goes16_vis') {
      if (!this.isConus(this.satelliteOverlayBounds)) {
        const statusEl = this.satelliteOverlayControls?.querySelector('.satellite-status') as HTMLSpanElement | null;
        if (statusEl) statusEl.textContent = 'Outside CONUS — fallback';
      } else {
        const layerId = 'realtime_goes16_abi_conus_band02_0p64um';
        candidates.push({
          url: this.buildGoesWmsUrl(layerId, bbox, clampedWidth, clampedHeight),
          label: 'GOES-16 Visible',
        });
        if (this.satelliteOverlayDateLabel) {
          this.satelliteOverlayDateLabel.textContent = 'Date: Latest (GOES)';
        }
        this.loadSatelliteImage(candidates);
        return;
      }
    }

    if (source === 'arcgis' || source === 'auto') {
      candidates.push({
        url: this.buildArcGisWorldImageryUrl(bbox, clampedWidth, clampedHeight),
        label: 'ArcGIS World Imagery',
      });
      if (this.satelliteOverlayDateLabel) {
        this.satelliteOverlayDateLabel.textContent = 'Date: Mosaic (unknown)';
      }
    }

    if ((source === 'naip' || (source === 'auto' && isUs && isSmall))) {
      candidates.push({
        url: this.buildNaipImageUrl(bbox, clampedWidth, clampedHeight),
        label: 'NAIP (USGS)',
      });
      if (this.satelliteOverlayDateLabel) {
        this.satelliteOverlayDateLabel.textContent = 'Date: NAIP (unknown)';
      }
    }

    if (source === 'sentinel' || (source === 'auto' && !this.satelliteOverlayState.live && isSmall)) {
      candidates.push({
        url: this.buildWmsUrl('https://tiles.maps.eox.at/wms', 's2cloudless', bbox, clampedWidth, clampedHeight),
        label: 'Sentinel-2 (EOX Cloudless)',
      });
      if (this.satelliteOverlayDateLabel) {
        this.satelliteOverlayDateLabel.textContent = `Date: ${this.satelliteOverlayState.live ? 'Latest (mosaic)' : date}`;
      }
    }

    if (source === 'gibs' || source === 'auto' || forceSource) {
      candidates.push({
        url: this.buildGibsWmsUrl(layerName, date, bbox, clampedWidth, clampedHeight, 'best'),
        label: 'NASA GIBS',
      });
      if (this.satelliteOverlayDateLabel) {
        this.satelliteOverlayDateLabel.textContent = `Date: ${date}`;
      }
    }
  ...
  private syncSatelliteOverlay(): void {
    ...
    // If selection is off-screen or too small, hide overlay
    const rect = this.container.getBoundingClientRect();
    const offscreen = left > rect.width || top > rect.height || (left + width) < 0 || (top + height) < 0;
    if (offscreen || width < 2 || height < 2) {
      this.satelliteOverlay.style.display = 'none';
      if (this.satelliteOverlayControls) this.satelliteOverlayControls.style.display = 'none';
      return;
    }

    this.satelliteOverlay.style.display = '';
    this.positionSatelliteOverlay();
    if (this.satelliteOverlayControls) this.satelliteOverlayControls.style.display = '';

    // Refresh the image to match current selection size
    this.updateSatelliteOverlayLayer();
  }

  private closeSatelliteOverlay(): void {
    this.satelliteOverlay?.remove();
    this.satelliteOverlay = null;
    this.satelliteOverlayControls?.remove();
    this.satelliteOverlayControls = null;
    this.satelliteOverlayImg = null;
    this.satelliteOverlayBounds = null;
    this.updateAttribution();
  }

  private syncSatelliteLayer(): void {
    const map = this.maplibreMap;
    if (!map) return;

    const sourceId = DeckGLMap.SATELLITE_SOURCE_ID;
    const layerId = DeckGLMap.SATELLITE_LAYER_ID;
  ...
  private buildLayers(): LayersList {
    ...
    // Construction activity layer (OSM)
    if (mapLayers.construction && this.constructionSites.length > 0) {
      layers.push(this.createConstructionLayer());
    }

    // Geo filter layer (OSM)
    if (mapLayers.geoFilters && this.geoFilterFeatures.length > 0) {
      layers.push(this.createGeoFilterLayer());
    }
    if (mapLayers.geoFilters && this.geoFilterMatches.length > 0) {
      layers.push(this.createGeoFilterMatchesLayer());
    }
  ...
  private createConstructionLayer(): ScatterplotLayer {
    return new ScatterplotLayer({
      id: 'construction-layer',
      data: this.constructionSites,
      getPosition: (d: ConstructionSite) => [d.lon, d.lat],
      getRadius: 15000,
      radiusMinPixels: 3,
      radiusMaxPixels: 10,
  ...
  private createGeoFilterLayer(): ScatterplotLayer {
    return new ScatterplotLayer({
      id: 'geo-filters-layer',
      data: this.geoFilterFeatures,
      getPosition: (d: GeoFilterFeature) => [d.lon, d.lat],
      getRadius: 1200,
      radiusMinPixels: 4,
      radiusMaxPixels: 10,
      getFillColor: (d: GeoFilterFeature) => GEOFILTER_COLORS[d.type] ?? [200, 200, 200, 200],
      stroked: true,
  ...
  private createGeoFilterMatchesLayer(): ScatterplotLayer {
    return new ScatterplotLayer({
      id: 'geo-filter-matches-layer',
      data: this.geoFilterMatches,
      getPosition: (d: GeoFilterMatch) => [d.lon, d.lat],
      getRadius: 2400,
      radiusMinPixels: 6,
      radiusMaxPixels: 14,
      getFillColor: GEOFILTER_MATCH_COLOR,
      stroked: true,
  ...
  private getTooltip(info: PickingInfo): { html: string } | null {
    ...
    const geoLabel = (type: string): string => {
      switch (type) {
        case 'roundabout': return 'Roundabout';
        case 'traffic_signals': return 'Traffic signals';
        case 'stop_sign': return 'Stop sign';
        case 'street_sign': return 'Street sign';
        case 'milestone': return 'Milestone';
        case 'level_crossing': return 'Level crossing';
        case 'bridge': return 'Bridge';
        default: return String(type || 'OSM feature');
      }
    };
    ...
    switch (layerId) {
      ...
      case 'construction-layer':
        return { html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/>${text(obj.category)} · ${text(obj.region)}</div>` };
      case 'geo-filters-layer': {
        const label = geoLabel(obj.type);
        const detail = obj.tags?.name || obj.tags?.ref || obj.tags?.traffic_sign || '';
        const detailLine = detail ? `<br/>${text(detail)}` : '';
        return { html: `<div class="deckgl-tooltip"><strong>${text(label)}</strong>${detailLine}</div>` };
      }
      case 'geo-filter-matches-layer': {
        const anchor = geoLabel(obj.anchorType);
        const matches = Array.isArray(obj.matchedTypes) ? obj.matchedTypes.map((t: string) => geoLabel(t)).join(', ') : '';
        const dist = obj.distanceKm ? `${Number(obj.distanceKm).toFixed(2)} km` : '';
        const detail = [matches, dist].filter(Boolean).join(' · ');
        const detailLine = detail ? `<br/>${text(detail)}` : '';
        return { html: `<div class="deckgl-tooltip"><strong>Geo match</strong><br/>${text(anchor)}${detailLine}</div>` };
      }
  ...
  private handleClick(info: PickingInfo): void {
    if (!info.object) {
      // Empty map click → country detection (satellite mode uses drag-select instead)
      if (info.coordinate && this.onCountryClick && !this.state.layers.satellite) {
  ...
  private createLayerToggles(): void {
    ...
    const layerConfig = SITE_VARIANT === 'tech'
      ? [
        ...
        { key: 'satellite', label: t('components.deckgl.layers.satellite'), icon: '&#128752;' },
      ]
      ...
      : [
        ...
        { key: 'pipelines', label: t('components.deckgl.layers.pipelines'), icon: '&#128738;' },
        { key: 'datacenters', label: t('components.deckgl.layers.aiDataCenters'), icon: '&#128421;' },
        { key: 'construction', label: t('components.deckgl.layers.construction'), icon: '&#127959;' },
        { key: 'geoFilters', label: t('components.deckgl.layers.geoFilters'), icon: '&#128269;' },
  ...
  public setConstructionSites(sites: ConstructionSite[]): void {
    this.constructionSites = sites;
    this.render();
  }

  public setGeoFilterResults(features: GeoFilterFeature[], matches: GeoFilterMatch[]): void {
    this.geoFilterFeatures = features;
    this.geoFilterMatches = matches;
    this.render();
  }
  ...
  public setLayerEnabled(layer: keyof MapLayers, enabled: boolean): void {
  ...
  public getBounds(): { west: number; south: number; east: number; north: number } | null {
    const bounds = this.maplibreMap?.getBounds();
  ...
  public toggleLayer(layer: keyof MapLayers): void {
    ...
    if (layer === 'satellite') {
      if (!this.state.layers.satellite) {
        this.closeSatelliteOverlay();
      }
      this.syncSatelliteLayer();
    }
    const toggle = this.container.querySelector(`.layer-toggle[data-layer="${layer}"] input`) as HTMLInputElement;
