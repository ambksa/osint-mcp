...
import type { MapLayers, Hotspot, NewsItem, InternetOutage, RelatedAsset, AssetType, AisDisruptionEvent, AisDensityZone, CableAdvisory, RepairShip, SocialUnrestEvent, MilitaryFlight, MilitaryVessel, MilitaryFlightCluster, MilitaryVesselCluster, NaturalEvent, CyberThreat, CableHealthRecord, ConstructionSite, GeoFilterFeature, GeoFilterMatch } from '@/types';
...
export class MapComponent {
  ...
  private createLayerToggles(): HTMLElement {
    ...
    const fullLayers: (keyof MapLayers)[] = [
      ...
      'cables', 'pipelines', 'outages', 'datacenters', 'construction', 'geoFilters',   // infrastructure
    ...
    const layerLabelKeys: Partial<Record<keyof MapLayers, string>> = {
      ...
      construction: 'components.deckgl.layers.construction',
      geoFilters: 'components.deckgl.layers.geoFilters',
      ...
      satellite: 'components.deckgl.layers.satellite',
  ...
  private renderOverlays(projection: d3.GeoProjection): void {
    ...
    if (this.state.layers.geoFilters) {
      this.renderGeoFilterMarkers(projection);
    }
    ...
    // Construction activity (OSM)
    if (this.state.layers.construction) {
      this.constructionSites.forEach((site) => {
        ...
        dot.style.backgroundColor = getCSSColor('--semantic-elevated');
        dot.title = `${site.name} — ${site.category}`;
  ...
  private renderGeoFilterMarkers(projection: d3.GeoProjection): void {
    const typeLabel = (type: GeoFilterFeature['type']): string => {
      switch (type) {
        ...
        default: return 'OSM feature';
      }
    };

    this.geoFilterFeatures.forEach((feature) => {
      const pos = projection([feature.lon, feature.lat]);
      if (!pos) return;

      const dot = document.createElement('div');
      dot.className = `geo-filter-marker geo-filter-${feature.type}`;
      dot.style.left = `${pos[0]}px`;
      dot.style.top = `${pos[1]}px`;
      dot.title = typeLabel(feature.type);
      this.overlays.appendChild(dot);
    });

    this.geoFilterMatches.forEach((match) => {
      const pos = projection([match.lon, match.lat]);
      if (!pos) return;

      const dot = document.createElement('div');
      dot.className = 'geo-filter-marker geo-filter-match';
      dot.style.left = `${pos[0]}px`;
      dot.style.top = `${pos[1]}px`;
      dot.title = `Geo match (${typeLabel(match.anchorType)})`;
      this.overlays.appendChild(dot);
    });
  }
  ...
  private static readonly ASYNC_DATA_LAYERS: Set<keyof MapLayers> = new Set([
    'natural', 'weather', 'outages', 'ais', 'protests', 'flights', 'military', 'techEvents', 'construction',
  ]);
  ...
  public getBounds(): { west: number; south: number; east: number; north: number } | null {
    // SVG fallback uses fixed cropped lat range; return approximate bounds.
    return { west: -180, south: -56, east: 180, north: 72 };
  }
