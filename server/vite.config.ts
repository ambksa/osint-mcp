...
function polymarketPlugin(): Plugin {
  ...
  const ALLOWED_INTEL_PATHS = new Set(['/markets', '/trades', '/book']);

  return {
    name: 'polymarket-dev',
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        if (req.url?.startsWith('/api/polymarket-intel')) {
          const url = new URL(req.url, 'http://localhost');
          const source = url.searchParams.get('source') || '';
          const path = url.searchParams.get('path') || '';

          const base = source === 'gamma' ? GAMMA_BASE : source === 'data' ? DATA_BASE : source === 'clob' ? CLOB_BASE : '';
          if (!base || !ALLOWED_INTEL_PATHS.has(path)) {
            res.statusCode = 400;
            res.setHeader('Content-Type', 'application/json');
            res.end(JSON.stringify({ error: 'Invalid source or path' }));
            return;
          }
          ...
          try {
            ...
            const resp = await fetch(target, { headers: { Accept: 'application/json' }, signal: controller.signal });
            clearTimeout(timer);
            if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
            const data = await resp.text();
            res.setHeader('Cache-Control', 'public, max-age=120');
            res.setHeader('X-Polymarket-Source', source);
            res.end(data);
          } catch {
...
function edgeApiPlugin(): Plugin {
  const EDGE_ROUTES: Record<string, string> = {
    '/api/headless': 'api/headless.js',
    '/api/geocode': 'api/geocode.js',
    '/api/satellite-snapshot': 'api/satellite-snapshot.js',
    '/api/telegram-feed': 'api/telegram-feed.js',
    '/api/rss-proxy': 'api/rss-proxy.js',
  };

  async function handleEdge(req: any, res: any) {
    const url = new URL(req.url, 'http://localhost');
    const route = url.pathname;
    const relPath = EDGE_ROUTES[route];
    if (!relPath) return false;

    const fileUrl = pathToFileURL(resolve(process.cwd(), relPath)).href;
    const mod = await import(fileUrl);
    const handler = mod?.default;
    if (typeof handler !== 'function') {
      res.statusCode = 500;
      res.end('Missing edge handler');
      return true;
    }
  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        try {
          const handled = await handleEdge(req, res);
          if (handled) return;
        } catch (error) {
          res.statusCode = 500;
          res.end(`Edge handler failed: ${error instanceof Error ? error.message : String(error)}`);
          return;
        }
        return next();
      });
...
export default defineConfig({
  envPrefix: ['VITE_', 'WORLDOSINT_', 'WS_'],
  define: {
  ...
  server: {
    ...
    port: 3000,
    strictPort: true,
    // Cloud Run service hostname must be explicitly allowed when Vite dev server
    // is used as runtime in container mode.
    allowedHosts: [
      'h1dr4-worldosint-dirlk4bbwa-ew.a.run.app',
      '.a.run.app',
    ],
    open: !isE2E,
