...
const IN_MEMORY_TTL = 60; // seconds

const inMemoryCache = new Map<string, { ts: number; data: SearchGdeltDocumentsResponse }>();
...
export async function searchGdeltDocuments(
  ...
  try {
    ...
    const cachedLocal = inMemoryCache.get(cacheKey);
    if (cachedLocal && Date.now() - cachedLocal.ts < IN_MEMORY_TTL * 1000) {
    ...
    const result = await cachedFetchJson<SearchGdeltDocumentsResponse>(
      ...
      async () => {
        ...
        try {
          ...
          if (rawText.includes('Please limit requests')) {
            throw new Error('GDELT rate limited (1 request per 5s).');
          }
    ...
    const finalResult = result || { articles: [], query, error: '' };
    inMemoryCache.set(cacheKey, { ts: Date.now(), data: finalResult });
    return finalResult;
  } catch (error) {
    return {
      articles: [],
